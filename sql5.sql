...
select count(amount) from payment where staff_id = 1;

select staff_id, count(amount) as total from payment group by staff_id;


-- kin staff_id nee 8000 se zayda payment ki hai
select staff_id, count(amount) as total from payment group by staff_id having total > 8000;


-- staff_id = 1 nee 4.99 ke kitne transcation kiye hai
select staff_id, count(amount) as total from payment where staff_id = 1 and amount = 4.99 ;


-- staff_id = 2 nee 2.99 ke kitne transcation kiye hai
select staff_id, count(amount) from payment where staff_id = 2 and amount = 2.99;


select staff_id, amount, count(payment_id) as total from payment group by staff_id, amount having count(payment_id) > 1000;


select staff_id, amount, count(payment_id) as total from payment group by staff_id, amount;
...
-- average quantity sold per product
select product_name, avg(quantity_sold) as average_quantity_sold from sales2 group by product_name;


-- number of distinct product sold per region
select region, count(distinct product_name) as number_of_distinct_product_sold from sales2 group by region;


-- Number of transactions per product per region
select region, product_name, count(price_per_unit) as number_of_transaction from sales2 group by region, product_name; 


-- Identify regions where more than 2 types of products were sold
select region, count(distinct product_name) as total from sales2 group by region having total > 2;


-- Total quantity and average price per unit of 'Mobile' across all regions 
select region, product_name, sum(quantity_sold) as total_quantity, avg(price_per_unit) as average_price from sales2 where product_name = 'mobile' group by region;


-- Find total sales revenue per year
select year(sale_Date) as years, sum(quantity_sold * price_per_unit) as total_sales_revenue from sales2 group by years;


-- Monthly breakdown per year (uses both YEAR and MONTH)
select year(sale_date) as years, month(sale_date) as months, sum(quantity_sold * price_per_unit) as total_revenue from sales2 group by years, months;
...
INSERT INTO sales2 (sale_id, sale_date, product_name, region, quantity_sold, price_per_unit) VALUES
(101, '2025-07-01', 'Laptop', 'East', 5, 55000.00),
(102, '2025-07-02', 'Laptop', 'West', 3, 56000.00),
(103, '2025-07-03', 'Mobile', 'East', 10, 15000.00),
(104, '2025-07-03', 'Laptop', 'East', 2, 55000.00),
(105, '2025-07-04', 'Mobile', 'South', 7, 14500.00),
(106, '2025-07-04', 'Tablet', 'North', 4, 22000.00),
(107, '2025-07-05', 'Mobile', 'East', 6, 15000.00),
(108, '2025-07-05', 'Tablet', 'North', 3, 21000.00);
