...
insert into test values(10, "abc"), (11, "def"), (12, "ghi"), (15, "pet"), (18, "ujjwal");
...
-- how can you optimize your sql query
-- Ans.=> Through the Indexes


-- What is Indexes
-- Indexes are nothing this is a mechanism which is used to access your data in a faster way from your database.


-- what is data page => your rows are stored in data pages.
-- divided in 3 parts:=> header, data record, offset. these 3 size will be 6kb (minimum)


-- indexes are of 2 types => 1.clustered index  2.non clustered index   

-- clustered index :=> it is a type of index which is used to store within the index and there are only one cluster index.
-- the actual data rows are physically rearranged on disk to match the index's order.
-- do not have more storage occupy
 
-- 1.unique index:=> no duplicate value allowed  2.all values are unique 3.  

 explain select * from test where name = "abc";


-- non clustered index :=>  in a non cluster index so rows are not physically arrange or sorted but it creates a separate table that references to original record.
-- storage more occupy 
...
-- composite index :=> in a index created a mutiple columns is called composite index.  


-- Assignment
-- Q. try to create a unique index on your table
-- Q. on a table one column will be primary and other one unique and check which is cluster and non cluster
-- Q. create index and visibility off that index and check that index will be working or not.
-- Q. can we apply indexing on million of records 
-- Ans. in the time of write we can't use indexes.

-- indexes are very very important for interview purpose
...
create unique index unique_email on orders (customer_email);

insert into orders values (1, 'Laptop', 'abc@gmail.com', '2025-09-17');

insert into orders values (2, 'Mobile', 'abc@gmail.com', '2025-09-18');
...
create table employee (
 ...
 create index indx_email on employee(email);
 
 show index from employee;
 
 alter table employee alter index indx_email invisible;
 
 show index from employee;
 
 explain select * from employee where email = 'abc@gmail.com';
 
  alter table employee alter index indx_email visible;
